...
def get_encoder(observation_type, nr_inputs, cnn_channels, batch_norm=True):
    ...
    if observation_type == '84x84':
        if batch_norm:
            enc.add(tf.keras.layers.Conv2D(cnn_channels[0], 8, strides=4))
            enc.add(tf.keras.layers.BatchNormalization())
            enc.add(tf.keras.layers.ReLU())
            enc.add(tf.keras.layers.Conv2D(cnn_channels[1], 4, strides=2))
            enc.add(tf.keras.layers.BatchNormalization())
            enc.add(tf.keras.layers.ReLU())
            enc.add(tf.keras.layers.Conv2D(cnn_channels[2], 3, strides=1))
            enc.add(tf.keras.layers.BatchNormalization()),
            enc.add(tf.keras.layers.ReLU())
            '''
            enc = nn.Sequential(
        ...
        if batch_norm:
            enc.add(tf.keras.layers.Conv2D(cnn_channels[0], 4, strides=2, padding='VALID'))
            enc.add(tf.keras.layers.BatchNormalization())
            enc.add(tf.keras.layers.ReLU())
            enc.add(tf.keras.layers.Conv2D(cnn_channels[1], 4, strides=2, padding='VALID'))
            enc.add(tf.keras.layers.BatchNormalization())
            enc.add(tf.keras.layers.ReLU())
            enc.add(tf.keras.layers.Conv2D(cnn_channels[2], 4, strides=1))
            enc.add(tf.keras.layers.BatchNormalization()),
            enc.add(tf.keras.layers.ReLU())
            '''
            enc = nn.Sequential(
        ...
        if batch_norm:
            enc.add(tf.keras.layers.Dense(cnn_channels[0]))
            enc.add(tf.keras.layers.BatchNormalization())
            enc.add(tf.keras.layers.ReLU())
            enc.add(tf.keras.layers.Dense(cnn_channels[1]))
            enc.add(tf.keras.layers.BatchNormalization())
            enc.add(tf.keras.layers.ReLU())
            '''
            enc = nn.Sequential(
        ...
        if batch_norm:
            ...
            enc.add(tf.keras.layers.BatchNormalization())
            enc.add(tf.keras.layers.ReLU())
            enc.add(tf.keras.layers.Conv2D(cnn_channels[1], 4, 2, padding='SAME'))
            enc.add(tf.keras.layers.BatchNormalization())
            enc.add(tf.keras.layers.ReLU())
            enc.add(tf.keras.layers.Conv2D(cnn_channels[2], 4, 2))
            enc.add(tf.keras.layers.BatchNormalization())
            enc.add(tf.keras.layers.ReLU())
            '''
