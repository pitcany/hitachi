...
\begin{document}
	...
	$$P(x_k|y_{1:N})=P(x_k|y_{1:k},y_{(k+1):N})\propto P(y_{(k+1):N}|x_k)P(x_k|y_{1:k})=b_{(k+1):N}\alpha_{1:k}$$ where $\alpha_{1:k} = P(x_k|y_{1:k})$ and $\beta_{(k+1):N}=P(y_{(k+1):N}|x_k)$. We call $\alpha$ and $\beta$ the forward and backward 'messages' respectively.
	...
	As mentioned before, we must constrain our set of state sequences to those that begin with a "perfect" state and end at a "failing" state. 
	
	Here we discuss how we can do this in practice:
	
	The idea is to skip parameter updates for $A_{ij}$ when $i<j$. This idea is inspired by the paper by Roweis discussing constrained HMM problems: https://cs.nyu.edu/~roweis/papers/sohmm.pdf.
