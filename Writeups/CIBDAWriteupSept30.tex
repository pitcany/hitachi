...
\begin{document}
	...
	$\alpha_{1:k}P_{ij}\beta_{jy_{k+1}}\beta_{k+1:N}$
	...
	$$\pi_{k|N}=P(x_k|y_{1:N})=P(x_k|y_{1:k},y_{k+1:N})\propto P(y_{k+1:N}|x_k)P(x_k|y_{1:k})=\beta_{k+1:N}\alpha_{1:k}$$ where $\alpha_{1:k} = P(x_k|y_{1:k})$ and $\beta_{k+1:N}=P(y_{k+1:N}|x_k)$. We call $\alpha$ and $\beta$ the forward and backward 'messages' respectively.
...
%	$C(i) = \frac{\sum_{k=1}^N \pi_{k|N}(i)y_k}{\sum_{k=1}^N \pi_{k|N}(i)}$ and $R(i)=$ (still computing this...).
	...
	To account for constraints on the final state \textit{and} the start state, we redefine $$\alpha_{1:k} = \frac{c_1\prod_{i=N}^{k} (P^{\top})^{-1} O_{i}^{-1}\alpha_{1:N} + c_2\prod_{i=2}^k (O_{i}P^{\top})\alpha_{1:1}}{2},$$ for $1<k<N$, where $c_1$ and $c_2$ are normalizing constants.
	
	Our backward message updates do not change.
